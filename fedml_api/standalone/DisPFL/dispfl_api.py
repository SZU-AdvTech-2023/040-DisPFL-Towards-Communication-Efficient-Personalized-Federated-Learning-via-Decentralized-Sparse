...
class dispflAPI(object):
        ...
        for round_idx in range(self.args.comm_round):
            ...
            for clnt_idx in range(self.args.client_num_in_total):
                ...
                if active_ths_rnd[clnt_idx] == 1:
                    # My-2
                    if self.args.my_way == 2:
                        w_local_mdl, w_per_globals[clnt_idx], new_mask = self._aggregate_func_2(
                            clnt_idx, self.args.client_num_in_total, self.args.client_num_per_round,
                            nei_indexs, w_per_mdls_lstrd, mask_pers_local, mask_pers_shared_lstrd,
                            client=self.client_list[clnt_idx], round=round_idx)

                    # Original
                    # TAG: 两个模型关系
                    # w_local_mdl = mask_pers * w_per_globals
                    # w_per_globals = avg(mask_pers * w_per_mdl of neighbours)
                    else:
                    ...
                    if self.args.my_way == 2:
                        new_mask = mask_pers_local[clnt_idx]

                # 聚合好模型后，更新shared mask
                if self.args.my_way == 2:
                    mask_pers_local[clnt_idx] = copy.deepcopy(new_mask)  # My-2: add this
                ...
                if self.args.my_way == 2:
                    # My-2
                    new_mask, w_local_mdl, updates_matrix[clnt_idx], training_flops, num_comm_params, tst_results = client.train2(
                          copy.deepcopy(w_local_mdl), copy.deepcopy(mask_pers_local[clnt_idx]), round_idx, copy.deepcopy(w_per_globals[clnt_idx]))
                    tst_results_ths_round.append(tst_results)
                    # NOTE: new_mask is not changed!
                else:
                    # Origin
                    new_mask, w_local_mdl, updates_matrix[clnt_idx], training_flops, num_comm_params, tst_results = client.train(
                        copy.deepcopy(w_local_mdl), copy.deepcopy(mask_pers_local[clnt_idx]), round_idx, copy.deepcopy(w_per_globals[clnt_idx]))
                    tst_results_ths_round.append(tst_results)
                ...
                if self.args.my_way == 2:
                    ...
                    mask_pers_local[clnt_idx] = copy.deepcopy(new_mask)  # My-2: remove this
    ...
    def _aggregate_func_2(self, cur_idx, client_num_in_total, client_num_per_round,
                          nei_indexs, w_per_mdls_lstrd, mask_pers, mask_pers_shared_lstrd,
                          client, round):
        print('Aggregation & Mask Changing')
        ...
        for k in w_tmp.keys():
            ...
            for clnt in nei_indexs:
                w_tmp[k] += w_per_mdls_lstrd[clnt][k] * mask_pers_shared_lstrd[clnt][k]
                # w_tmp[k] += torch.from_numpy(count_mask[k]) * w_per_mdls_lstrd[clnt][k]
            w_tmp[k] = w_tmp[k] / len(nei_indexs)
            # sum / n  小，说明出现次数少，一般不聚合
            # (sum(nei) / n = w_tmp, mask-one
            # topN(mask-one | (mask_per\(1-f_decay)权值最大的)), N = count_ons(mask_per)*f_decay, 从中重新选取
            # 保留权值最大的一部分（使用self的值），另一部分从聚合结果中选择最大的（w_tmp的值，如果已经被保留了-定为self，则作为随机机会，从其他未被选择的位置随机选择（使用w_tmp的值））
            # Note: w_tmp 也聚合了 self 的值
            # 先修改 mask 再训练
            #

        new_mask, num_remove = client.fire_mask(mask_pers[cur_idx], w_per_mdls_lstrd[cur_idx], round)
        w_p_g = copy.deepcopy(w_tmp)
        for k in new_mask:
            # 保留权值最大的一部分（使用self的值）
            w_p_g[k][new_mask[k] == 1] = w_per_mdls_lstrd[cur_idx][k][new_mask[k] == 1]

        _num_random_ = []
        _num_total_ = []
        for k in new_mask:
            # regrow: 从聚合结果中选择最大的
            sort_tmp, idx = torch.sort(torch.abs(w_tmp[k]).view(-1).to(self.device), descending=True)
            idx = idx[:num_remove[k]]  # 取一定量
            num_random = torch.sum(new_mask[k].view(-1)[idx] == 1).item()  # 已经有的，作为随机机会
            new_mask[k].view(-1)[idx] = 1
            # w_p_g[k].view(-1)[idx] = w_tmp[k].view(-1)[idx]

            _num_random_.append(num_random)
            _num_total_.append(torch.sum(new_mask[k].view(-1) == 1).item())

            # 随机采样
            if num_random != 0:
                temp = torch.where(new_mask[k] == 0, torch.ones_like(new_mask[k]), torch.zeros_like(new_mask[k]))
                idx_random = torch.multinomial(temp.flatten().to(self.device), num_random, replacement=False)
                new_mask[k].view(-1)[idx_random] = 1
                # w_p_g[k].view(-1)[idx_random] = w_tmp[k].view(-1)[idx_random]

        self.logger.info("$$$$$$$$ Mask")
        self.logger.info("RANDOM = %s" % str(_num_random_))
        self.logger.info("TOTAL_ = %s" % str(_num_total_))

        for k in new_mask:
            w_tmp[k] = w_tmp[k] * new_mask[k]
        return w_tmp, w_p_g, new_mask
