...
class Client:
    ...
    def train2(self, w, masks, round, w_global):
        print('train2')
        ...
        self.logger.info(
            "test acc on this client before {} / {} : {:.2f}".format(tst_results['test_correct'], tst_results['test_total'], tst_results['test_acc']))
        ...
        tst_results = self.model_trainer.test(self.local_test_data, self.device, self.args)
        self.logger.info(
            "test acc on this client after {} / {} : {:.2f}".format(tst_results['test_correct'], tst_results['test_total'], tst_results['test_acc']))
    ...
    def train(self, w, masks,round, w_global):
        # downlink params
        num_comm_params = self.model_trainer.count_communication_params(w)
        self.model_trainer.set_model_params(w)
        self.model_trainer.set_masks(masks)
        ...
        if not self.args.static:
            if not self.args.dis_gradient_check:
                print('screen-gradients')
                if self.args.my_way == 1:
                    self.model_trainer.set_model_params(w_global)  # My-one: 使用全局模型
                gradient = self.model_trainer.screen_gradients(self.local_training_data, self.device)
                if self.args.my_way == 1:
                    self.model_trainer.set_model_params(weights)  # My-one: 复原
            else:
                print('no-gradients')
            masks, num_remove = self.fire_mask(masks, weights, round)
            masks = self.regrow_mask(masks, num_remove, gradient)
            # # Warn-up: 使用全局模型
            # for name in masks:
            #     print('weight-zeros:(before warn-up)', torch.count_nonzero(weights[name] * masks[name]))
            #     weights[name] = torch.where(masks[name] > 0, torch.where(weights[name] != 0, weights[name], w_global[name]), 0)
            #     print('weight-zeros:(after warn-up)', torch.count_nonzero(weights[name] * masks[name]))
            # self.model_trainer.set_model_params(weights)
    ...
    def regrow_mask(self, masks,  num_remove, gradient=None):
        ...
        for name in masks:
            # if name not in public_layers:
                # if "conv" in name:
                if not self.args.dis_gradient_check:
                    # TAG: (从未加入mask的权重中)取最大的(与裁剪的数量相同) NOTE: 每层稀疏度是固定的
